...
import org.eclipse.lyo.adapter.tdb.serviceproviders.ServiceProviderCatalogSingleton;
...
public class ServiceProviderService
{
    ...
    public void getHtmlServiceProvider(@PathParam("serviceProviderId") final String serviceProviderId)
    {
    	...
    	for (Service service : services) {
			if(service.getDomain().toString().contains("resources")){
				service.setAbout(URI.create("http://localhost:" + OSLC4JTDBApplication.portNumber + "/oslc4jtdb/services/" + serviceProviderId + "/resources"));
			}
...
//	        httpServletRequest.setAttribute("service", services[0]);
	        ...
	        RequestDispatcher rd = httpServletRequest.getRequestDispatcher("/triplestore/serviceprovider_html.jsp");
    		try {
